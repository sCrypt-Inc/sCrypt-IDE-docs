...
**sCrypt IDE** 自带一个功能强调的调试器。你可以用它来调试 * *.scrypt* 源代码，如果您购买了高级付费功能，您还可以用它来进行比特币脚本级别的调试。
...
与其它语言不同的是， 启动 **sCrypt IDE** 的 debugger 必须配置启动参数，而一般语言（比如c++/javascript)则不需要。这是 **sCrypt** 语言的结构特性决定的，如下图：
...
创建launch.json
---------------
打开调试视图，如果当前工作区没有创建 **launch.json** ，会提示您创建 **launch.json** 文件。
...
name                  是     配置名称
program               是     要调试的源代码文件，可以使用vscode的工作区代号 ``${workspaceFolder}``，也可以使用绝对路径
constructorArgs       是     合约的构造参数实例
pubFunc               是     要调试的 ``public`` 方法名称
pubFuncArgs           是     运行 ``public`` 方法的参数实例，即解锁脚本
txContext             否     运行方法所需的交易上下文，具体定义参照下表 
asmArgs               否     `汇编变量 <https://scryptdoc.readthedocs.io/zh_CN/latest/asm.html>`_ 实例
===============    ======== ======================================================================================
...
字段                必须    说明
===============    ======== ======================================================================================
hex                  是     序列化的当前交易，即解锁脚本所在交易
inputIndex           是     解锁脚本对应input所在当前交易中的索引
inputSatoshis        是     解锁脚本对应input中锁定的satoshis
...
在启动调试器之前，我们首先设置断点。断点是可靠调试的最基本和最重要的功能，若代码编辑器中打开了文件，则可通过单击代码行左侧的边缘来设置断点，或者按 **F9**。
接下来我们按 **F5** 或者 点击 **运行和调式** 侧边栏面板中的“开始调试”按钮，调试器将运行至它遇到的第一个断点。
...
在您通过上述操作浏览代码时，左侧调试视图的中的 **变量面板** 和 **调用堆栈面板** 中的数据会实时地变化，如果你在 **监视面板** 添加了表达式或者变量，将会实时地变化。
...
在某个变量上右键单击，会出现 **复制值** 菜单，您可以直接复制变量的值到剪切板中。
