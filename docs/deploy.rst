...
1. 填写调用函数所需的实参。
2. 添加交易输出，对于有状态合约，一般会限制合约的输出脚本，所以需要手动管理所有交易输出。目前支持的输出类型有四种：
...
1. ``Sig`` 类型
    ...
    部署 ``P2PKH`` 合约的时候会有一个 ``Publickey Hash`` 的输入框。默认值是你设置的私钥的 ``Publickey Hash``。 你可以把它修改成别的私钥的 ``Publickey Hash``。
    如果你修改了，那么当你解锁它的时候，就需要有对应的私钥的签名。别担心，IDE 允许你使用对应的私钥来签名交易。签名输入框中的默认签名是用默认私钥计算的。
    点击输入框的右边的 [Sig] 按钮，在弹出的输入框中输入正确的私钥，就可以计算出正确的签名了。

2. ``SigHashPreimage`` 类型
   
    IDE 会根据你添加的输出，设置的交易相关参数，比如 ``SigHashType``, ``nLockTime``， 自动计算出交易第一个输入的 preimage 原象。
