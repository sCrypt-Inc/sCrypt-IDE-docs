...
**IDE** 提供一个通用的 UI 交互界面，只需简单的填写相关参数，就能一键部署合约，点击按钮就能调用合约方法，无须编写一行代码。
...
打开部署合约配置视图后， 填写合约构造函数的各个参数，以及合约锁定 ``satoshis`` 余额。如果是状态合约，还需要填写初始的状态，然后点击 **Deploy** 按钮，就可以看到部署成功对应的 ``txid``。
...
点击合约实例上的构造方法，则可查看构造方法调用时的实参。点击 ``public`` 方法，右边则会出现方法的调用视图。
